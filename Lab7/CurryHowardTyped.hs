...
orElim (Right b) f g =  g b
orElim (Left a) f g = f a
...
ax2 :: (a -> b) -> (a -> (b -> c)) -> a -> c
ax2 f g a = implIntro(\f -> implIntro(\g -> implIntro(\a -> implIntro(implIntro g a) (implIntro f a)))) f g a

ax3 :: a -> b -> And a b
ax3 a b = implIntro(\a -> implIntro (\b -> andIntro a b)) a b
